...
public class RoundImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        if (mDisableCircularTransformation || mType == TYPE_NORMAL) {
            super.onDraw(canvas);
            return;
        }
        ...
        } else if (mType == TYPE_SQUARE) {
            float left = getPaddingLeft();
            float top = getPaddingTop();
            float right = getMeasuredWidth() - getPaddingRight();
            float bottom = getMeasuredHeight() - getPaddingBottom();
            canvas.drawRoundRect(new RectF(left, top, right, bottom), mCorner, mCorner, mBitmapPaint);
    ...
    private void setup() {
        ...
        mCircleBackgroundPaint.setDither(true);
